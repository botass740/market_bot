...
async def main():
    ...
    before = await pm.get_product_count(PlatformCode.WB)
    removed = await pm.trim_to_target(PlatformCode.WB, TARGET)
    after = await pm.get_product_count(PlatformCode.WB)

    print(f"WB before={before}, removed={removed}, after={after}")
